...
function new_strategy_slide_2(window_data: WindowData): Slide {
    ...
    const slide_window = new_slide_window(window_data, "strategy",
        element("main", { className: "titled" },
            ...
            jfx.new_panel(
                new_bullet_list(
                    element("strong", {}, "Estratégia:"),
                    new_bullet_list(
                        "declara uma interface comum aos algoritmos suportados;",
                    ),

                    element("strong", {}, "Estratégia Concreta:"),
                    new_bullet_list(
                        "implementa a interface da estratégia;"
                    ),

                    element("strong", {}, "Contexto:"),
                    new_bullet_list(
                        "usa a interface da estratégia implementada por alguma estratégia concreta,",
                        "contém e/ou recebe uma referência a uma estratégia."
                    ),
                ),
                image_cell
            )
...
async function new_strategy_slide_3(window_data: WindowData): Promise<Slide> {
    const slide_window = (
        new_slide_window(window_data, "strategy",
            element("main", { className: "titled" },
                element("h1", {}, "Caneta Colorida"),
                await new_code_block_fetch("./assets/code/strategy/pen/pen.go", "go"),
            )
        )
    )
...
async function new_strategy_slide_4(window_data: WindowData): Promise<Slide> {
    const packet = "Hello, World!\n"

    const web = (
        element("code", { className: "code-block web" },
            element("span", {},
                ...packet.split("").map(ch => {
            ...
            element("div", { className: "file" },
                element("img", { alt: "image-icon", src: "./assets/images/web.png" }),
                element("span", {}, "google.com"),
            ),
        )
    )

    const file = element("div", { className: "file" },
        element("img", { alt: "image-icon", src: "./assets/images/image-icon.png" }),
        element("span", {}, "a.txt"),
    )

    const file_system = (
        element("code", { className: "code-block file-system" },
            file,
            element("div", { className: "file" },
                element("img", { alt: "image-icon", src: "./assets/images/image-icon.png" }),
                element("span", {}, "strategy.go"),
            )
        )
    )
    ...
    const message = element("span", {},
        element("span", { style: { color: "#f00" } }, packet),
        element("br"), "> "
    )

    const terminal = (
        element("code", { className: "code-block terminal" },
            command, message
        )
    )
    ...
    const slide_window = (
        new_slide_window(window_data, "strategy",
            element("main", { className: "titled" },
                element("h1", {}, "A Interface Escritora"),
                new_two_columns(
                    await new_code_block_fetch("./assets/code/strategy/writer/writer.go", "go"),
                    element("div", { className: "result" }, terminal, web, file_system),
                )
            )
        )
    )

    slide_window.animation(animation_strategy_slide_4.bind(null,
        terminal, command, web, file_system, file, message
    ))

    return slide_window
}

function* animation_strategy_slide_4(...elements: HTMLElement[]) {
...
async function new_strategy_slide_5(window_data: WindowData): Promise<Slide> {
    const slide_window = (
        new_slide_window(window_data, "strategy",
            element("main", { className: "titled" },
                ...
                new_two_columns(
                    element("article", {},
                        ...
                        element("p", {},
                            "Esse comportamento permite interoperabilidade com estratégias já existentes - como o ",
                            jfx.new_button("Comparator"), "."),
                    ),
                    await new_code_block_fetch("./assets/code/strategy/Main.java", "java"),
                )
            )
        )
    )
...
async function new_strategy_slide_6(window_data: WindowData): Promise<Slide> {
    const slide_window = (
        new_slide_window(window_data, "strategy",
            element("main", { className: "titled" },
                element("h1", {}, "Estratégias Modernas"),
                jfx.new_panel(
                    ...
                    await new_code_block_fetch("./assets/code/strategy/map.c", "c"),
                )
            )
        )
    )
