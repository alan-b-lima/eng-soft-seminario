...
import { new_slide_window, new_two_columns } from "./common.js";
...
function new_strategy_slide() {
    ...
    const slide_window = (new_slide_window(WINDOW_DATA, option, jfx.new_panel(jfx.new_field("Definir uma família de algoritmos, encapsular cada uma, e fazê-los intercambiáveis. " +
        "Strategy permite que o algoritmo varie de forma independente do cliente que o usa."), element("span", {}, "~ Gang of Four"))));
    slide_window.element().classList.add(`${option}-slide`);
    return slide_window;
}
function new_references_slide() {
    const option = "references";
    const slide_window = (new_slide_window(WINDOW_DATA, option, new_two_columns(element("div", {}, "Casey Muratori"), element("div", {}, "Gang Of Four"))));
    slide_window.element().classList.add(`${option}-slide`);
    return slide_window;
}
